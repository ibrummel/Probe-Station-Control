...
parser.add_argument('-t', '-T', '--temp', '--temperature', dest='step_temp', nargs='?', default=35, type=float,
                    help='Choose the temperature to set on the control instrument. Will set to 35C if not provided.')
    ...
    if args.ctrl_inst == "None":
        ...
        if temperature_controller.PID_enabled:
            ...
            while True:
                ...
                chamber_T.append(float(hotplate_robot.get_temp()))
                print('Current Temperature: {}'.format(chamber_T[-1]), end='\r')
                count += 1
                sleep(1)
                if count >= int(args.ramp_time * 60):
                    ...
                    if np.std(chamber_T) < args.stdev_tol:
                        chamber_T = []
                        break
            print('')
        ...
        for i in range(0, int(args.dwell * 60)):
            if count % args.stab_int == 0:
                ...
                chamber_T.append(curr_temp)
                sleep(0.05)
            count += 1
            time_left = str(timedelta(seconds=int(args.dwell * 60) - i))
            print("Checking stability at {temp}. Current Temperature: {curr_temp}, Time Remaining: {time}".format(temp=args.step_temp, curr_temp=curr_temp, time=time_left), end='\r')
            sleep(0.956)
        print('')
        ...
        chamber_avg = np.mean(chamber_T)
        chamber_stdev = np.std(chamber_T)
        print(chamber_T, chamber_avg, chamber_stdev)
